...
def all_plot_FKS(data, title, filename, trange, deltaE, xrange=None, yrange=None, loca=0, ylable='need a label!'):
    ...
    for i in trange:
        ax.errorbar(data[i][:, 0]*a, -data[i][:, 1]*2.1753**2/deltaE, data[i][:, 2]*2.1753**2/deltaE, label=r'$n_t=$'+str(i).rjust(2, '0'), **style)
...
all_plot_FKS(fkstdC, 'test', 'test', np.arange(4, 28, 2), 1, xrange=[0, 1.4], yrange=[5, -14], ylable='test', loca=4)
