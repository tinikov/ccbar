...
for type in $(ls $SAMPLE_DIR); do
  ...
  $BIN_DIR/mean -l $TIMESITES -o $O_DIR/corr/$type -j -t $SAMPLE_DIR/$type/2pt.*
...
for type in $(ls $SAMPLE_DIR); do
  ...
  $BIN_DIR/effmass -l $TIMESITES -d $EFF_DIR/$type $SAMPLE_DIR/$type/2pt.*

  mkdir -p $O_DIR/effmass/binary
  $BIN_DIR/mean -l $TIMESITES -o $O_DIR/effmass/exp.$type -j -t $EFF_DIR/$type/exp.*
  $BIN_DIR/mean -l $TIMESITES -o $O_DIR/effmass/csh.$type -j -t $EFF_DIR/$type/csh.*
